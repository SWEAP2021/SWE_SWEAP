...
function handleChangeCost(e) {
  userInputCost = e.target.value;
} //지정가 입력창에 입력 받기
function changeCost() {
  change = true;
} //지정가로 가격 변동
function clickToPortfolio(e) {
...
class BuyStockPage extends Component {
  ...
  //매수를 진행하는 함수
  buyStock = () => {
    if (this.state.userAsset >= this.state.totalCost) {
      //현재 사용자의 자산이 매수하려는 총 가격보다 많을 경우
      this.requestOrder();
  ...
  requestOrder = () => {
    if (this.state.inputcost == this.state.nowcost) {
      //사용자 입력값이 현재 시장가와 동일하면
      this.setState({ result: "주문 완료" });
      this.updateMystockList(); //매수 체결
      this.updateAccount();
    } else if (this.state.inputcost > this.state.nowcost) {
      //사용자 입력값이 현재 시장가보다 크면
  ...
  updateMystockList = () => {
    this.hasStock();
    if (this.state.userQuantity >= 0) {
      //해당 주식을 사용자가 가지고 있으면
      const post = {
        //주식 개수만 업데이트
        query:
          "UPDATE MYSTOCKLIST SET HoldingQuantity=" +
          (this.state.userQuantity + this.state.quantity) +
          " WHERE UserID='" +
          this.state.userid +
          "' AND HoldingStockName='" +
          this.state.stockName +
          "';", //mysql로 전송할 쿼리 문
      };
      fetch("http://localhost:4000/SQL1", {
      ...
      //해당 주식을 사용자가 가지고 있지않으면
      const post = {
        //주식 리스트에 추가
        query:
          "INSERT INTO MYSTOCKLIST(UserID,HoldingQuantity,HoldingStockName) VALUES ('" +
          this.state.userid +
          "'," +
          this.state.quantity +
          ",'" +
  ...
  hasStock = () => {
    const post = {
      query:
        "SELECT * FROM MYSTOCKLIST WHERE HoldingStockName='" +
        this.state.stockName +
        "' AND UserID='" +
        this.state.userid +
        "';", //mysql로 전송할 쿼리 문
  ...
  updateAccount = () => {
    const post = {
      query:
        "UPDATE ACCOUNT SET TotalAsset=" +
        (this.state.userTotalAsset + this.state.totalCost) +
        ", Asset=" +
        (this.state.userAsset - this.state.totalCost) +
        " WHERE UserID='" +
        this.state.userid +
        "';", //mysql로 전송할 쿼리 문
    };
  ...
  updateTransaction = () => {
    const post = {
      query:
        "INSERT INTO TRANSACTION(StockName, Quantity,TransactionDate,TransactionType, UserID,InputPrice,Result) VALUES ('" +
        this.state.stockName +
        "'," +
        this.state.quantity +
        ",NOW(),'SELL','" +
  ...
  CalTotalCost = () => {
    this.state.totalCost = this.state.inputcost * this.state.quantity;
    return this.state.totalCost;
  }; //총 가격=현재가격*현재 개수
  AddCost = () => {
    this.setState({ inputcost: this.state.inputcost + 100 });
  }; //가격 +100
  minusCost = () => {
    if (this.state.inputcost != 0) {
      this.setState({ inputcost: this.state.inputcost - 100 });
    }
  }; //가격 -100
  AddQuantity = () => {
  ...
  closePopup = () => {
    ...
    if (change == true) {
      this.setState({ inputcost: Number(userInputCost) });
    }
    change = false;
  }; //팝업 닫음 이때, 가격을 변동시킴
}
